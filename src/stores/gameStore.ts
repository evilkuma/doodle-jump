...
class GameStore {
  ...
  addPlatform(
    ...
    y: number,
    width: number,
    type: "normal" | "moving" | "breakable" = "normal"
  ) {
  ...
  updatePlatforms(_playerY: number) {
    this.platforms = this.platforms.filter(
      (platform) => platform.y > this.cameraY - this.VIEWPORT_HEIGHT / 2
    );

    const highestVisibleY = this.cameraY + this.VIEWPORT_HEIGHT;
    while (this.highestPlatformY < highestVisibleY) {
      this.generatePlatformAtHeight(
        this.highestPlatformY + this.PLATFORM_SPAWN_RATE
      );
    }
  }
  ...
  checkPlatformCollision(): boolean {
    ...
    for (const platform of this.platforms) {
      ...
      if (this.playerVelocity.y <= 0.1) {
        // Разрешаем небольшую положительную скорость
        if (
  ...
  setPlayerVelocity(x: number, y: number) {
    ...
    this.playerVelocity.x = Math.max(
      -this.MOVE_SPEED,
      Math.min(this.MOVE_SPEED, x)
    );
